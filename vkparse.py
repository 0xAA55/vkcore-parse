...
def to_rust(outfile, parsed):
	...
	vk_s_impl.write('\t\tlet mut has_VK_KHR_buffer_device_address = false;\n')
	vk_s_impl.write('\t\tlet mut has_VK_EXT_buffer_device_address = false;\n')
	vk_s_impl.write('\t\tfor extension in extensions.iter() {\n')
	vk_s_impl.write('\t\t\tif extension == "VK_KHR_buffer_device_address" {\n')
	vk_s_impl.write('\t\t\t\thas_VK_KHR_buffer_device_address = true;\n')
	vk_s_impl.write('\t\t\t\tif has_VK_EXT_buffer_device_address {continue;}\n')
	vk_s_impl.write('\t\t\t} else if extension == "VK_EXT_buffer_device_address" {\n')
	vk_s_impl.write('\t\t\t\thas_VK_EXT_buffer_device_address = true;\n')
	vk_s_impl.write('\t\t\t\tif has_VK_KHR_buffer_device_address {continue;}\n')
	vk_s_impl.write('\t\t\t}\n')
	vk_s_impl.write('\t\t\tc_strings.push(CString::new(&**extension).unwrap());\n')
	vk_s_impl.write('\t\t}\n')
	vk_s_impl.write('\t\t#[cfg(feature = "all_extensions")]\n')
	vk_s_impl.write('\t\tlet ext_pointers: Vec<*const i8> = c_strings.iter().map(|s|s.as_ptr()).collect();\n')
	vk_s_impl.write('\t\t#[cfg(not(feature = "all_extensions"))]\n')
	vk_s_impl.write('\t\tlet ext_pointers: Vec<*const i8> = Vec::new();\n')
	vk_s_impl.write('\t\t#[cfg(not(feature = "all_extensions"))]\n')
	vk_s_impl.write('\t\tlet extensions: Vec<String> = Vec::new();\n')
	vk_s_impl.write('\t\t#[cfg(feature = "validation_layer")]\n')
	vk_s_impl.write('\t\tlet layers = ["VK_LAYER_KHRONOS_validation"];\n')
