...
def to_rust(outfile, parsed):
	...
	def process_version(version, verdata, f):
		...
		for func in funcs:
			...
			if ret_type == 'VkResult':
				...
				t_impl.write(f'\t\tprocess_catch("{func}", catch_unwind(||((self.{func_snake})({", ".join(param_call)}))))\n')
				vk_traits.write(f'\t\tprocess_catch("{func}", catch_unwind(||((self.{snake_version}.{func_snake})({", ".join(param_call)}))))\n')
			t_impl.write('\t}\n')
			vk_traits.write('\t}\n')
	...
	with open(outfile, 'w') as f:
		...
		f.write('fn process_catch<T>(funcname: &\'static str, ret: std::thread::Result<T>) -> Result<T> {\n')
		f.write('\tmatch ret {\n')
		f.write('\t\tOk(ret) => Ok(ret),\n')
		f.write('\t\tErr(_) => {\n')
		f.write('\t\t\tErr(VkError::NullFunctionPointer(funcname))\n')
		f.write('\t\t}\n')
		f.write('\t}\n')
		f.write('}\n')
		f.write('\n')
