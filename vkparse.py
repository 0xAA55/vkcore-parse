...
def to_rust(outfile, parsed):
	...
	vk_struct.write(f'\tpub app_info: VkApplicationInfo,\n')
	vk_s_impl.write('impl VkCore {\n')
	vk_c_impl.write('impl Default for VkApplicationInfo {\n')
	vk_c_impl.write('\tfn default() -> Self {\n')
	vk_c_impl.write('\t\tlet app_name = CString::new("<APPLICATION NAME>").unwrap();\n')
	vk_c_impl.write('\t\tlet engine_name = CString::new("<ENGINE_NAME>").unwrap();\n')
	vk_c_impl.write('\t\tSelf {\n')
	vk_c_impl.write('\t\t\tsType: VkStructureType::VK_STRUCTURE_TYPE_APPLICATION_INFO,\n')
	vk_c_impl.write('\t\t\tpNext: null(),\n')
	vk_c_impl.write('\t\t\tpApplicationName: app_name.as_ptr(),\n')
	vk_c_impl.write('\t\t\tapplicationVersion: vk_make_version(1, 0, 0),\n')
	vk_c_impl.write('\t\t\tpEngineName: engine_name.as_ptr(),\n')
	vk_c_impl.write('\t\t\tengineVersion: vk_make_version(1, 0, 0),\n')
	vk_c_impl.write('\t\t\tapiVersion: VK_API_VERSION_1_4,\n')
	vk_c_impl.write('\t\t}\n')
	vk_c_impl.write('\t}\n')
	vk_c_impl.write('}\n')
	vk_c_impl.write('\n')
	vk_c_impl.write('impl Clone for VkCore {\n')
	vk_c_impl.write('\tfn clone(&self) -> Self {\n')
	vk_c_impl.write('\t\tlet mut c_strings: Vec<CString> = Vec::with_capacity(self.extensions.len());\n')
	vk_c_impl.write('\t\tfor extension in self.extensions.iter() {\n')
	vk_c_impl.write('\t\t\tc_strings.push(CString::new(&**extension).unwrap());\n')
	vk_c_impl.write('\t\t}\n')
	vk_c_impl.write('\t\tlet mut ext_pointers: Vec<*const i8> = Vec::with_capacity(c_strings.len());\n')
	vk_c_impl.write('\t\tfor s in c_strings.iter() {\n')
	vk_c_impl.write('\t\t\text_pointers.push(s.as_ptr());\n')
	vk_c_impl.write('\t\t}\n')
	vk_c_impl.write('\t\tlet create_info = VkInstanceCreateInfo {\n')
	vk_c_impl.write('\t\t\tsType: VkStructureType::VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO,\n')
	vk_c_impl.write('\t\t\tpNext: null(),\n')
	vk_c_impl.write('\t\t\tflags: 0,\n')
	vk_c_impl.write('\t\t\tenabledLayerCount: 0,\n')
	vk_c_impl.write('\t\t\tpApplicationInfo: &self.app_info,\n')
	vk_c_impl.write('\t\t\tppEnabledLayerNames: null(),\n')
	vk_c_impl.write('\t\t\tenabledExtensionCount: ext_pointers.len() as u32,\n')
	vk_c_impl.write('\t\t\tppEnabledExtensionNames: ext_pointers.as_ptr()\n')
	vk_c_impl.write('\t\t};\n')
	vk_c_impl.write('\t\tlet mut instance: VkInstance = null();\n')
	vk_c_impl.write('\t\tself.vkCreateInstance(&create_info, null(), &mut instance).unwrap();\n')
	vk_c_impl.write('\t\tSelf {\n')
	vk_c_impl.write('\t\t\tinstance,\n')
	vk_c_impl.write('\t\t\textensions: self.extensions.clone(),\n')
	vk_c_impl.write('\t\t\tapp_info: self.app_info,\n')
	...
	vk_s_impl.write('\t\tvk_result_conv("vkEnumerateInstanceExtensionProperties", vkEnumerateInstanceExtensionProperties(null(), &mut count as *mut _, null_mut()))?;\n')
	vk_s_impl.write('\t\tlet mut extensions: Vec<VkExtensionProperties> = Vec::with_capacity(count as usize);\n')
	vk_s_impl.write('\t\tvk_result_conv("vkEnumerateInstanceExtensionProperties", vkEnumerateInstanceExtensionProperties(null(), &mut count as *mut _, extensions.as_mut_ptr()))?;\n')
	...
	def process_version(version, verdata, f):
		...
		vk_c_impl.write(feature_indent_3)
		vk_c_impl.write(f'\t\t\t{snake_version}: self.{snake_version},\n')
		snakes = {}
		if len(funcs):
			traits.write('\n')
			vk_traits.write('\n')
