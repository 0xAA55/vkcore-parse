...
def to_rust(outfile, parsed):
	...
	vk_s_impl.write('\t\t\tpApplicationInfo: &app_info as *const _,\n')
	vk_s_impl.write('\t\t\tppEnabledLayerNames: null(),\n')
	vk_s_impl.write('\t\t\tenabledExtensionCount: count,\n')
	vk_s_impl.write('\t\t\tppEnabledExtensionNames: extensions.as_ptr() as *const _\n')
	vk_s_impl.write('\t\t};\n')
	...
	vk_s_impl.write('\t\t}\n')
	#vk_s_impl.write('\t\tlet vkGetInstanceProcAddr = get_proc_address(null(), "vkGetInstanceProcAddr");\n')
	#vk_s_impl.write('\t\tif vkGetInstanceProcAddr == null() {\n')
	#vk_s_impl.write('\t\t\tpanic!("Initialize Vulkan failed: couldn\'t get the address of `vkGetInstanceProcAddr()`");\n')
	#vk_s_impl.write('\t\t}\n')
	#vk_s_impl.write('\t\tlet vkGetInstanceProcAddr: PFN_vkGetInstanceProcAddr = unsafe{transmute(vkGetInstanceProcAddr)};\n')
	#vk_s_impl.write('\t\tlet mut get_proc_address = |instance, funcname|vkGetInstanceProcAddr(instance, CString::new(funcname).unwrap().as_ptr()) as *const c_void;\n')
	vk_s_impl.write('\t\t\n')
	vk_s_impl.write('\t\tSelf {\n')
	vk_s_impl.write('\t\t\tinstance,\n')
	vk_s_impl.write('\t\t\textensions: extensions.into_iter().map(|prop|unsafe {CStr::from_ptr(prop.extensionName.as_ptr())}.to_string_lossy().to_string()).collect(),\n')
